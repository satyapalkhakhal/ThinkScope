...
import { Menu, X, Search, Bell, User, ChevronRight, Sun, Moon } from 'lucide-react';
...
export default function Header() {
  ...
  useEffect(() => {
    async function loadCategories() {
      try {
        ...
        if (data) {
          setCategories(data.slice(0, 6)); // Limit to 6 categories for navigation
        }
      } catch (error) {
  ...
  return (
    <>
      {/* Top Bar */}
      <div className={`text-sm py-2 border-b transition-colors ${theme === 'dark' ? 'bg-primary-900 text-white border-primary-700' : 'bg-gray-100 text-gray-900 border-gray-200'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className={`hidden md:block ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
      ...
      {/* Main Header */}
      <header className={`sticky top-0 z-50 shadow-md border-b transition-colors ${
        theme === 'dark' 
          ? 'bg-primary-850 border-primary-700' 
          : 'bg-white border-gray-200'
      } ${isScrolled ? 'py-2' : 'py-3'} duration-300`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/" className="flex-shrink-0 flex items-center space-x-2">
              ...
              <span className={`text-xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>ThinkScope</span>
            </Link>
            ...
            <div className="hidden md:flex flex-1 max-w-xl mx-8">
              <div className="relative w-full">
                <input
                  ...
                  className={`w-full px-4 py-2 border rounded-l-md focus:outline-none focus:ring-2 focus:ring-accent-500 transition-colors ${
                    theme === 'dark'
                      ? 'bg-primary-800 border-primary-600 text-white placeholder-gray-400'
            ...
            <div className="flex items-center space-x-4">
              <button 
                ...
                className={`p-2 rounded-lg transition-colors ${
                  theme === 'dark'
                    ? 'text-gray-400 hover:text-accent-500 hover:bg-primary-800'
                    : 'text-gray-600 hover:text-accent-500 hover:bg-gray-100'
                }`}
          ...
          <nav className="hidden md:flex items-center space-x-1 overflow-x-auto mt-3 pb-2">
            {mainNavigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className={`px-3 py-2 text-sm font-medium whitespace-nowrap transition-colors relative group ${
                  theme === 'dark'
      ...
      >
        <div className="px-4 pt-16 pb-3 space-y-1">
          {mainNavigation.map((item) => (
            <Link
              ...
              className={`flex items-center justify-between px-3 py-3 text-base font-medium rounded-lg transition-colors ${
                theme === 'dark'
                  ? 'text-gray-300 hover:text-accent-500 hover:bg-primary-800'
          ...
          <div className={`pt-4 mt-4 border-t ${theme === 'dark' ? 'border-primary-700' : 'border-gray-200'}`}>
